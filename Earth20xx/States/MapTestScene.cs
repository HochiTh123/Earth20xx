...
namespace Earth20xx.States
{
    public class MapTestScene : Engine.IScene
    {
        ...
        public void Init()
        {
            if (MainClass.Instance.CurrentSession == null)
                MainClass.Instance.CurrentSession = new GameData.Session();
            MainClass.Instance.CurrentSession.SetPlayerCount(2);
            MainClass.Instance.CurrentSession.Players[0].PlayerColor = Color.Blue;
            MainClass.Instance.CurrentSession.Players[1].PlayerColor = Color.Green;
            if (MainClass.Instance.Map == null)
            {
                MainClass.Instance.Map = new GameData.Map();
            }
            MainClass.Instance.Map.Init();
            MainClass.Instance.Map.Generate(100, 200);
